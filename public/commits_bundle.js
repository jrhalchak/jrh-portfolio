...
/***/ function(module, exports, __webpack_require__) {
	...
	var _commitsModel = __webpack_require__(2);

	var _commitsModel2 = _interopRequireDefault(_commitsModel);
	...
	var gitApi = 'https://api.github.com/search/repositories?q=%20+fork:true+user:jrhalchak';
	...
	(0, _commitsModel2.default)(repos, function (commitLists) {
	  ...
	  commitData = {
	    ...
	    data: {
	      labels: commits.slice().map(function (c) {
	        return c.date;
	      }),
	      datasets: [{
	        ...
	        pointRadius: 1,
	        pointHitRadius: 10,
	        data: commits.slice().map(function (c) {
	          return c.count;
	        })
	      }]
	    },
	    options: {
	      ...
	      ctx = $('.js-commitsChart'),
	      commitsChart = new _chart2.default(ctx, commitData);
	});
...
/***/ function(module, exports) {
	...
	exports.default = function (repositories, callback) {
	  var _$;

	  var promises = [],
	      commitLists = [];

	  repositories.forEach(function (c, i, a) {
	    promises.push(getCommits(c).then(function (data) {
	      var tempList = [];

	      data.forEach(function (cur, ind, arr) {
					...
					switch (this.options.hover.mode) {
						...
						case 'dataset':
							for (var i = 0; i < this.lastActive.length; i++) {
								if (this.lastActive[i]) this.data.datasets[this.lastActive[i]._datasetIndex].controller.removeHoverStyle(this.lastActive[i], this.lastActive[i]._datasetIndex, this.lastActive[i]._index);
							}
							break;
						default:
					...
					switch (this.options.hover.mode) {
						...
						case 'dataset':
							for (var j = 0; j < this.active.length; j++) {
								if (this.active[j]) this.data.datasets[this.active[j]._datasetIndex].controller.setHoverStyle(this.active[j]);
							}
							break;
						default:
						// Don't change anything
	...
	module.exports = function (Chart) {
		...
		Chart.defaults.global.legend = {
			...
			labels: {
				...
				// lineJoin :
				// lineWidth :
				generateLabels: function generateLabels(data) {
				...
				if (this.options.display) {
					...
					if (this.isHorizontal()) {
						...
						helpers.each(this.legendItems, function (legendItem, i) {
							...
							var itemOrDefault = function itemOrDefault(item, defaulVal) {
	...
	module.exports = function (Chart) {
		...
		Chart.controllers.line = Chart.DatasetController.extend({
			...
			updateBezierControlPoints: function updateBezierControlPoints() {
				...
				helpers.each(this.getDataset().metaData, function (point, index) {
					var controlPoints = helpers.splineCurve(helpers.previousItem(this.getDataset().metaData, index)._model, point._model, helpers.nextItem(this.getDataset().metaData, index)._model, point._model.tension);
			...
			draw: function draw(ease) {
				...
				if (this.chart.options.showLines) this.getDataset().metaDataset.transition(easingDecimal).draw();
...
/***/ function(module, exports) {
	...
	module.exports = function (Chart) {
		...
		Chart.controllers.radar = Chart.DatasetController.extend({
			...
			updateBezierControlPoints: function updateBezierControlPoints() {
				helpers.each(this.getDataset().metaData, function (point, index) {
					var controlPoints = helpers.splineCurve(helpers.previousItem(this.getDataset().metaData, index, true)._model, point._model, helpers.nextItem(this.getDataset().metaData, index, true)._model, point._model.tension);
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(!locales[name]&&typeof module!=='undefined'&&module&&module.exports){try{oldLocale=globalLocale._abbr;__webpack_require__(34)("./"+name); // because defineLocale currently also sets the global locale, we
	...
	module.exports = function (Chart) {
		...
		Chart.elements.Point = Chart.Element.extend({
			...
			draw: function draw() {
				...
				if (_typeof(vm.pointStyle) === 'object' && (vm.pointStyle.toString() === '[object HTMLImageElement]' || vm.pointStyle.toString() === '[object HTMLCanvasElement]')) {
